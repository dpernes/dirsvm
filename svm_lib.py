...
class Svm(BaseEstimator, ClassifierMixin):
    def __init__(self, model_function_forward, model_function_backward,
        ...
        if model_function_post_train_step is None:
            ...
            self.model_function_post_train_step = \
                model_function_post_train_step
    ...
    def init_adam(self, rho1=None, rho2=None):
        ...
        for param in self.model_params:
            self.model_params_1st_moment[param] = \
                np.zeros_like(self.model_params[param])
                ...
                if self.optimizer == 'SGD':
                    (self.model_params[param],
                     self.model_params_vel[param]) = \
                        self.sgd_momentum(self.model_params[param],
                                          grads[param],
                                          self.model_params_vel[param])
...
class AsyTriSvm(Svm):
    def __init__(self, is_directional, weight_scale=1e-3, C=1, lr=1e-1,
        ...
        super(AsyTriSvm, self).__init__(
            ...
            num_epochs=num_epochs, lr=lr, optimizer=optimizer,
            lr_decay=lr_decay, momentum=momentum, rho1=rho1, rho2=rho2)
    ...
    def fit(self, x, y_enc, batch_size=None, init_params=True, init_optim=True,
        ...
        ret = (super(AsyTriSvm, self)
               .fit(x_norm, y_enc, batch_size=batch_size,
...
class AsyTriKernelSVM(Svm):
    def __init__(self, is_directional, weight_scale=1e-3, C=1, lr=1e-1,
        ...
        super(AsyTriKernelSVM, self).__init__(
            ...
            optimizer=optimizer, lr_decay=lr_decay, momentum=momentum,
            rho1=rho1, rho2=rho2, num_epochs=num_epochs)
    ...
    def kernels(self, x, z):
        ...
        arg_tri = (x_dir.reshape(N, 1, -1)
                   - z_dir.reshape(1, M, -1)) % (2*np.pi)
